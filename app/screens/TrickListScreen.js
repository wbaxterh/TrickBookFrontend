...
const loadTricks = async () => {
    ...
    return (
        <Screen>
                ...
                renderItem={({item}) => 
                    <Trick trick={item} name={item.name} checked={item.checked} onPress={() => navigation.navigate(routes.TRICKDETAILS, item)} 
                        renderRightActions={() => (<TrickDelete onPress={() => handleDelete(item)} />)}
                        renderLeftActions={() => (<TrickEdit onPress={() => navigation.navigate(routes.EDITTRICK, item)} />)}
                    />
                    }
                ...
                />
                <AppButton style={styles.btn} foregroundColor={colors.dark} backgroundColor={colors.primary} title="+ Add Trick" onPress={() => navigation.navigate(routes.ADDTRICK, trickList)} />
        </Screen>
    );
}

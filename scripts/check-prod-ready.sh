...
# ios/ and android/ are generated by prebuild and legitimately contain the
# Maps key at build time, so they are excluded here and covered by check 9.
SECRETS_FOUND=$(grep -rnE "sk_live|AKIA[A-Z0-9]{16}|AIza[0-9A-Za-z_-]{30,}" \
  --include="*.ts" --include="*.tsx" --include="*.js" \
  --exclude-dir=node_modules --exclude-dir=.expo --exclude-dir=ios --exclude-dir=android \
  . 2>/dev/null || true)
...
# 9. Native projects must stay generated (Continuous Native Generation).
# A tracked ios/ or android/ tree freezes the Google Maps key and stale
# config into git. Only meaningful where a git checkout exists (CI, local);
# EAS build workers receive an archive without .git, so skip there.
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  TRACKED_NATIVE=$(git ls-files ios android 2>/dev/null || true)
  if [ -n "$TRACKED_NATIVE" ]; then
    echo "FAIL: ios/ or android/ files are tracked in git (run: git rm -r --cached ios android):"
    echo "$TRACKED_NATIVE" | head -10
    ERRORS=$((ERRORS + 1))
  else
    echo "PASS: Native projects are not tracked (generated by prebuild)"
  fi

  TRACKED_KEYS=$(git grep -nE "AIza[0-9A-Za-z_-]{30,}" -- . ':!.env.example' 2>/dev/null || true)
  if [ -n "$TRACKED_KEYS" ]; then
    echo "FAIL: Google API key committed in tracked files:"
    echo "$TRACKED_KEYS"
    ERRORS=$((ERRORS + 1))
  else
    echo "PASS: No Google API key in tracked files"
  fi
else
  echo "WARN: Not a git checkout; skipping tracked-file checks (expected on EAS build workers)"
fi
